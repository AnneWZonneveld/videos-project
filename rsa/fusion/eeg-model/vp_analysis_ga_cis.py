"""
Calculated confidence intervals for variance partitioning on group level.
...
shm_name = 'vp_GA_cis'
results = calc_cis_rw2_mp(eeg_rdms=eeg_rdms, design_matrices=design_matrices, jobarr_id=args.jobarr_id, n_cpus=args.n_cpus, shm_name=shm_name, its=10000)
...
file_path = res_folder + f'vp_cis_results_unstructured.pkl'
...
for t in range(len(eeg_data['times'])):
     for var in vars:
          ci_lower[var].append(results[t][var][0])
          ci_upper[var].append(results[t][var][1])


results_rs = (ci_lower, ci_upper)
...
file_path = res_folder + f'vp_cis_results.pkl'
